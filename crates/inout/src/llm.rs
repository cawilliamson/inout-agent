...
pub enum StreamEvent {
    ...
    Content(String),
    /// reasoning/thinking delta (e.g. glm-5.2 reasoning_effort=high)
    Reasoning(String),
...
impl CallCost {
    pub(crate) fn compute(rates: &ModelRates, usage: &OpenAiUsage) -> Self {
        ...
        let cache_write_tokens =
            usage.prompt_tokens_details.as_ref().map(|d| d.cache_write_tokens).unwrap_or(0);
...
async fn fetch_model_rates(
    ...
    let resp = client.get(&url).header(AUTHORIZATION, format!("Bearer {api_key}")).send().await?;
    let parsed: GatewayModelsResponse = serde_json::from_str(&resp.text().await?)?;
    let mut map = HashMap::new();
    for m in parsed.data {
        let pricing = m.providers.first().and_then(|p| p.pricing.as_ref());
...
fn parse_price(s: &Option<String>) -> f64 {
    s.as_deref().and_then(|v| v.trim().parse::<f64>().ok()).unwrap_or(0.0)
}
...
impl LlmClient for HttpLlmClient {
    async fn complete(&self, req: LlmRequest) -> anyhow::Result<LlmResponse> {
        ...
        if let Some(usage) = &parsed.usage {
            let _ = self.record_cost(&req.model, usage);
        }

        let msg =
            parsed.choices.first().ok_or_else(|| anyhow::anyhow!("empty choices"))?.message.clone();
        let tool_calls = msg
            .tool_calls
            .unwrap_or_default()
            .into_iter()
...
fn parse_sse_event(raw: &str) -> Vec<SseEvent> {
    // skip comment lines (': ping') and empty data
    let data: Vec<&str> = raw.lines().filter_map(|l| l.strip_prefix("data: ")).collect();
    ...
    // reasoning delta — emitted first so the thinking appears before the answer
        ...
        .or_else(|| delta.get("reasoning_content").and_then(|r| r.as_str()))
    {
        if !reasoning.is_empty() {
            out.push(SseEvent::DeltaReasoning(reasoning.to_string()));
        }
    }

    // content delta — may coexist with reasoning on the same chunk
    if let Some(content) = delta.get("content").and_then(|c| c.as_str()) {
        if !content.is_empty() {
            out.push(SseEvent::DeltaContent(content.to_string()));
        }
    }
    ...
    if let Some(tcs) = delta.get("tool_calls").and_then(|t| t.as_array()) {
        if let Some(tc) = tcs.first() {
            ...
            let name =
                function.and_then(|f| f.get("name")).and_then(|n| n.as_str()).map(String::from);
            let arguments = function
                .and_then(|f| f.get("arguments"))
                .and_then(|a| a.as_str())
                .map(String::from);
            out.push(SseEvent::DeltaTool { index, id, name, arguments });
        }
    }
...
fn build_openai_body(req: &LlmRequest) -> Value {
    ...
    // glm-5.2 exposes a thinking budget via reasoning_effort; high = deeper chain-of-thought.
    if req.model.starts_with("glm-5.2") {
